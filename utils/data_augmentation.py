...
def random_flip(img, masks, masks2, u=0.5):
    if np.random.random() < u:
        img = tf.keras.preprocessing.image.flip_axis(img, 1)
        for i in range(masks.shape[0]):
            masks[i] = tf.keras.preprocessing.image.flip_axis(masks[i], 1)
        for i in range(masks2.shape[0]):
            masks2[i] = tf.keras.preprocessing.image.flip_axis(masks2[i], 1)
    if np.random.random() < u:
        img = tf.keras.preprocessing.image.flip_axis(img, 0)
        for i in range(masks.shape[0]):
            masks[i] = tf.keras.preprocessing.image.flip_axis(masks[i], 0)
        for i in range(masks2.shape[0]):
            masks2[i] = tf.keras.preprocessing.image.flip_axis(masks2[i], 0)
